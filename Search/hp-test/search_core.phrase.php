...
class SearchCore {
	...
	public function __construct() {
		...
		$this->_tagList = array("acne skin", "acne scar");
	}
	...
	public function _pf_get_matched_tags($s) {
		$txt = strtolower($s);
		$foundTags = array();
		foreach ($this->_tagList as &$tag) {
			if (strpos($txt, $tag) !== false) {
				array_push($foundTags, $tag);
				if (sizeof($foundTags) >= 2) {
					break;
				}
			}
		}
	...
	public function _pf_get_post_query($s, $searchType, $offset, $count) {
		...
		if ($searchType == 'random') {
			...
			$foundTags = $this->_pf_get_matched_tags($s);		
			if (sizeof($foundTags) > 0) {
